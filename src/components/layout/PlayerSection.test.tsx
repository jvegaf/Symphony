...
vi.mock('../WaveformCanvas', () => ({
  WaveformCanvas: ({ trackId, onSeek }: { trackId?: string; onSeek: (time: number) => void }) => (
    <button 
      type="button"
...
describe('PlayerSection', () => {
  ...
  const createMockAudioPlayer = (overrides: Partial<ReturnType<typeof useAudioPlayerHook.useAudioPlayer>> = {}) => ({
    play: mockPlay,
    pause: mockPause,
    resume: mockResume,
    stop: mockStop,
  ...
  beforeEach(() => {
    ...
    // Mock useAudioPlayer
    vi.mocked(useAudioPlayerHook.useAudioPlayer).mockReturnValue(createMockAudioPlayer());
  ...
  describe('Renderizado básico', () => {
    it('debería renderizar sin track', () => {
      ...
      expect(screen.getByText('Sin pista seleccionada')).toBeInTheDocument();
      expect(screen.getByText('—')).toBeInTheDocument(); // Artist placeholder
    });
  ...
  describe('Análisis de beatgrid', () => {
    ...
    it('debería mostrar estado de loading durante análisis', () => {
      ...
      const analyzeButton = screen.getByRole('button', { name: /hourglass_top/i });
      expect(analyzeButton).toBeDisabled();
  ...
  describe('Cue Points', () => {
    ...
    it('debería mostrar botones de cue con estado correcto', () => {
      ...
      expect(cue1Button).toHaveAttribute('title', expect.stringContaining('Cue 1: 0:30'));
      ...
      // Cue 3 no existe
      const cue3Button = screen.getAllByRole('button').find(btn => 
        btn.querySelector('.material-icons')?.textContent === 'add'
      );
    ...
    it('debería crear cue point al hacer click en botón vacío', async () => {
      ...
      const cue1Button = screen.getAllByRole('button').find(btn => 
        btn.querySelector('.material-icons')?.textContent === 'add'
      );
    ...
    it('no debería crear/eliminar cue points sin track', () => {
      ...
      const cueButtons = screen.getAllByRole('button').filter(btn => 
        btn.textContent?.includes('add') || btn.getAttribute('class')?.includes('bg-blue-500')
      );
  ...
  describe('Estados de error', () => {
    ...
    it('debería manejar errores en análisis de beatgrid', async () => {
      ...
      await waitFor(() => {
        expect(consoleError).toHaveBeenCalledWith('Error analizando beatgrid:', expect.any(Error));
      });
    ...
    it('debería manejar errores en creación de cue point', async () => {
      ...
      if (cue1Button) {
        ...
        await waitFor(() => {
          expect(consoleError).toHaveBeenCalledWith('Error creando cue point:', expect.any(Error));
        });
      }
