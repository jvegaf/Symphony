...
fn test_full_rating_import_flow() {
    ...
    let metadata = extractor.extract_metadata(test_file).expect("Failed to extract metadata");
    ...
    assert_eq!(metadata.rating, Some(5), "El archivo test.mp3 debe tener rating de 5 estrellas");
    ...
    assert_eq!(track.rating, Some(5), "El Track debe preservar el rating de los metadatos");
    ...
    symphony_lib::db::migrations::run_migrations(&conn)
        .expect("Failed to run migrations");
    
    let track_id = symphony_lib::db::queries::insert_track(&conn, &track)
    ...
    assert_eq!(saved_track.rating, Some(5), 
