...
use crate::audio::{
    AudioError, 
    AudioResult,
    constants::{WAVEFORM_WINDOW_SIZE, WAVEFORM_PEAKS_PER_PACKET, WAVEFORM_PEAK_METHOD},
    dsp,
};
...
pub async fn generate_waveform_streaming(
    ...
    // 1. Verificar cache en DB
    {
        ...
        if let Ok(Some(waveform)) = queries::get_waveform(&conn, &track_id) {
            ...
            let peaks: Vec<f32> = serde_json::from_str(&data_str)
                .unwrap_or_default();
            
            log::info!("📤 Emitting waveform:complete event (from cache) - {} peaks", peaks.len());
            
            app.emit("waveform:complete", WaveformCompletePayload {
    ...
    tokio::task::spawn(async move {
        ...
        match result {
            Ok(peaks) => {
                ...
                // Emitir evento final
                log::info!("📤 Emitting waveform:complete event (from generation) - {} peaks", peaks.len());
                let _ = app_clone.emit("waveform:complete", WaveformCompletePayload {
            ...
            Err(e) => {
                ...
                let _ = app_clone.emit("waveform:error", WaveformErrorPayload {
                    track_id: track_id_clone.clone(),
                    error: e.to_string(),
                });
            }
...
pub async fn cancel_waveform_generation(
    track_id: &str,
    state: Arc<WaveformState>,
) -> bool {
    let mut active = state.active_generations.write().await;
...
async fn generate_and_stream_peaks(
    ...
    loop {
        ...
        if packet_count % WAVEFORM_PEAKS_PER_PACKET == 0 {
            ...
            log::info!("📊 Progress: {:.0}% - {} packets, {} peaks", progress * 100.0, packet_count, peaks.len());
            ...
            if !new_peaks.is_empty() {
                log::info!("   Enviando {} nuevos peaks (total acumulado: {})", new_peaks.len(), peaks.len());
    ...
    log::info!("🏁 Loop finished - processed {} packets total", packet_count);
    ...
    log::info!("✅ Normalization complete - returning {} peaks", normalized.len());
